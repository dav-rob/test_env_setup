...
normalize_path() {
    echo "$1" | sed -e 's#/\+#/#g' -e 's#/\./\?#/#g' -e 's#\(/[^/]\+\)/\.\./\?#\1#g'
}
...
get_backup_path() {
    ...
    if [ -d "$source" ]; then
        ...
        echo "$(normalize_path "$BACKUP_DIR/$relative_path")"
    else
        # For files, strip all directory prefixes
        local filename=$(basename "$source")
        echo "$(normalize_path "$BACKUP_DIR/$filename")"
    fi
}
...
process_directory() {
    ...
    while IFS= read -r -d '' file; do
        ...
        local dest_file=$(normalize_path "$dest/$relative_path")
        
        if files_differ "$file" "$dest_file"; then
            if [ $in_dry_run -eq 1 ]; then
                [ $VERBOSE -eq 1 ] && log_message "Would backup file: $file"
                ((FILE_UPDATES++))
            else
                [ $VERBOSE -eq 1 ] && log_message "Backing up file: $file"
...
check_deletions() {
    ...
    for item in "${FILES_TO_BACKUP[@]}"; do
        if [ -d "$item" ]; then
            while IFS= read -r -d '' file; do
                # Get clean paths relative to home directory
                local relative_path=$(normalize_path "${file#$HOME/}")
                current_paths+=("$relative_path")
                [ $VERBOSE -eq 1 ] && log_message "Adding to current paths: $relative_path"
            done < <(cd "$HOME" && find "${item#$HOME/}" -type f -print0)
        else
