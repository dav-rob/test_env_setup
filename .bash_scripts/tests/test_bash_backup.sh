...
# Define directories
BACKUP_SCRIPT_DIR="$HOME/.bash_scripts"
...
run_tests() {
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "3:0" "Verbose dry-run" || test_failed=1  # Expect 3 files (2 standalone + 1 in directory)
    
    # Test 1.2: Non-verbose dry-run push
    echo "Running Test 1.2: Non-verbose dry-run push..."
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "3:0" "Non-verbose dry-run" || test_failed=1  # Expect 3 files
    
    # Test 1.3: Verbose live push
    echo "Running Test 1.3: Verbose live push..."
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "3:0" "Verbose live push" || test_failed=1  # Expect 3 files
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "Non-verbose live push" || test_failed=1
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "New root file - Verbose dry-run" || test_failed=1

    # Test 2.2: Non-verbose dry-run
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "New root file - Non-verbose dry-run" || test_failed=1

    # Test 2.3: Non-verbose live push
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "New root file - Non-verbose live push" || test_failed=1

    # Test 2.4: Verbose live push (should show no changes)
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "New root file - Verbose live push" || test_failed=1
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "New subdir file - Verbose dry-run" || test_failed=1

    # Test 3.2: Non-verbose dry-run
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "New subdir file - Non-verbose dry-run" || test_failed=1

    # Test 3.3: Verbose live push
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "New subdir file - Verbose live push" || test_failed=1

    # Test 3.4: Non-verbose live push (should show no changes)
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "New subdir file - Non-verbose live push" || test_failed=1
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "Modified root file - Verbose dry-run" || test_failed=1

    # Test 4.2: Non-verbose dry-run
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "Modified root file - Non-verbose dry-run" || test_failed=1

    # Test 4.3: Non-verbose live push
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "Modified root file - Non-verbose live push" || test_failed=1

    # Test 4.4: Verbose live push (should show no changes)
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "Modified root file - Verbose live push" || test_failed=1
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "Modified subdir file - Verbose dry-run" || test_failed=1

    # Test 5.2: Non-verbose dry-run
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "Modified subdir file - Non-verbose dry-run" || test_failed=1

    # Test 5.3: Verbose live push
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "1:0" "Modified subdir file - Verbose live push" || test_failed=1

    # Test 5.4: Non-verbose live push (should show no changes)
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "Modified subdir file - Non-verbose live push" || test_failed=1
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "2:0" "New folder with files - Verbose dry-run" || test_failed=1

    # Test 6.2: Non-verbose dry-run
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "2:0" "New folder with files - Non-verbose dry-run" || test_failed=1

    # Test 6.3: Verbose live push
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "2:0" "New folder with files - Verbose live push" || test_failed=1

    # Test 6.4: Non-verbose live push (should show no changes)
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "New folder with files - Non-verbose live push" || test_failed=1
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:1" "Remove root file - Verbose dry-run" || test_failed=1

    # Test 7.2: Non-verbose dry-run
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:1" "Remove root file - Non-verbose dry-run" || test_failed=1

    # Test 7.3: Non-verbose live push
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:1" "Remove root file - Non-verbose live push" || test_failed=1

    # Test 7.4: Verbose live push (should show no changes)
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "Remove root file - Verbose live push" || test_failed=1
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:1" "Remove subdir file - Verbose dry-run" || test_failed=1

    # Test 8.2: Non-verbose dry-run
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:1" "Remove subdir file - Non-verbose dry-run" || test_failed=1

    # Test 8.3: Verbose live push
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:1" "Remove subdir file - Verbose live push" || test_failed=1

    # Test 8.4: Non-verbose live push (should show no changes)
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "Remove subdir file - Non-verbose live push" || test_failed=1
    ...
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:2" "Remove folder - Verbose dry-run" || test_failed=1

    # Test 9.2: Non-verbose dry-run
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -d --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:2" "Remove folder - Non-verbose dry-run" || test_failed=1

    # Test 9.3: Non-verbose live push
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:2" "Remove folder - Non-verbose live push" || test_failed=1

    # Test 9.4: Verbose live push (should show no changes)
    output=$("$BACKUP_SCRIPT_DIR/bash_backup.sh" -v --backup-dir "$BACKUP_DIR" --files "$FILES_STRING")
    verify_changes "$output" "0:0" "Remove folder - Verbose live push" || test_failed=1
